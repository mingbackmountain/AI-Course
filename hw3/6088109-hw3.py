...
def simulated_annealing(env, agent, init_temp=25.0, temp_step=-0.1, max_iters=10000):
    ...
    for __ in range(max_iters):
        ...
        if temperature < 0:
            break
        previousNeighbor: list[CPAgent] = cur_agent.neighbors()
        nextNeighbor: CPAgent = np.random.choice(previousNeighbor, 1)[0]
        cur_r = simulate(env, [cur_agent])[0]
        delta_e = simulate(env, [nextNeighbor])[0] - cur_r
        if delta_e > 0:
            cur_agent = nextNeighbor
            cur_r = simulate(env, [cur_agent])[0]
        elif np.random.rand() <= pow(math.e, delta_e/temperature):
            cur_agent = nextNeighbor
            cur_r = simulate(env, [cur_agent])[0]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        agent = CPAgent(w1=np.array(
            [0.0011, 0.0909, 0.0688, 0.189]), b1=np.array([0.0056]))
        # Hill Climbing search cannot solve this case, but sideway move limit at 10 will solve this.
        # agent = CPAgent(w1=np.array(
        #     [0.0155, 0.0946, 0.0225, 0.0975]), b1=np.array([-0.0628]))
